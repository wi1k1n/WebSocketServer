...
namespace WebSocketServer
{
                ...
                while (listen)
                {
                    try
                    {
                        ...
                        } else
                        {
                            ...
                            packet.Opcode = !awP ? ByteManager.constructByte(rawData[0], 0, 3) : packet.Opcode; // Стоит проверка для того, чтобы не перезаписать нулями уже записанный опкод
    ...
    public class WSDataReceivedEventArgs
    {
        private List<WSPacket> packetFragments;
